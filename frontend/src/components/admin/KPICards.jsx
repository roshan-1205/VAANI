  ...
  return (
    <div className="grid grid-cols-4 gap-6 max-xl:grid-cols-2 max-sm:grid-cols-1">
      {kpis.map((kpi, index) => (
        <motion.div
          key={index}
          className="bg-dark p-6 rounded-xl flex gap-4 items-start shadow-[0_4px_16px_rgba(0,0,0,0.1)] transition-all hover:-translate-y-1 hover:shadow-[0_8px_24px_rgba(0,0,0,0.15)]"
        ...
        >
          ...
          <div className="flex-1">
            <div className="text-white/60 text-[13px] font-medium mb-2">{kpi.label}</div>
            <div className="text-white text-[28px] font-bold font-display mb-1">{kpi.value}</div>
            <div className={`text-success text-xs font-semibold ${kpi.isNegative ? 'text-success' : ''}`}>
              {kpi.trend}
            </div>
          </div>
        </motion.div>
